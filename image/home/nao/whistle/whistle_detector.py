...
class Config(object):
    ...
    def __init__(self):
        ...
        self.nWhistleOkaySpectra = 7  # ~149ms at 48 kHz
        self.nWhistleMissSpectra = 4  # ~85ms at 48 kHz
...
class WhistleState(object):
    ...
    def interrogate(self, complex_spectrum):
        ...
        num_buckets = 10
        grow_size = int((end - begin) / num_buckets)
...
class SoundReceiverMixin(object):
    ...
    def to_sound_data(audio_buffer, num_channels, samples_per_channel):
        sound_data_interlaced = numpy.frombuffer(audio_buffer, dtype=numpy.int16)
...
if not TEST_MODE:
    ...
    class SoundReceiverModule(SoundReceiverMixin):
        def __init__(self):
            ...
            pcm = alsaaudio.PCM(type=alsaaudio.PCM_CAPTURE, device='mixed_mono')
...
if __name__ == "__main__":
    ...
    RETRY = 3
    while not TEST_MODE and RETRY > 0:
        RETRY -= 1
        try:
            run_on_nao()
